...
class LoadingButton @JvmOverloads constructor(
    ...
    init {
        if (attrs != null) {
            ...
            mColorPrimary = ta.getInt(R.styleable.LoadingButton_lb_btnColor, Color.parseColor("#2C5C8C"))
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        when (mCurrentState) {
            STATE_BUTTON, STATE_ANIMATION_STEP1 -> {
                ...
                if (mCurrentState == STATE_BUTTON) {
                    canvas.drawText(mText, (width - mTextWidth) / 2, (viewHeight + mTextHeight) / 2 , mTextPaint)
